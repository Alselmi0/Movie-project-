...
function Video(props, ref) {
    ...
    if (video?.type === "Trailer") {
        if (language === "ar-SA") {
            video_type_to_print = "عرض";
        }
    } else if (video?.type === "Teaser") {
        if (language === "ar-SA") {
            video_type_to_print = "إعلان";
        }
    } else if (video?.type === "Featurette") {
        if (language === "ar-SA") {
            video_type_to_print = "فيلم عن";
        }
    } else if (video?.type === "Clip") {
        if (language === "ar-SA") {
            video_type_to_print = "مقطع فيديو";
        }
    } else if (video?.type === "Behind the Scenes") {
        if (language === "ar-SA") {
            video_type_to_print = "وراء الكواليس";
        }
    }
                        ...
                        {props.showVideo && (
                            <>
                                <div className="flex-container">
                                    {video_type_to_print ? (
                                        <div className="flex-item left">{video_type_to_print}</div>
                                    ) : (
                                        <div className="flex-item left"></div>
                                    )}
                                    <div className="flex-item right">
                                        <svg onClick={() => props.setShowVideo(false)} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                            <path fill="#DC3545" d="M3 16.74L7.76 12L3 7.26L7.26 3L12 7.76L16.74 3L21 7.26L16.24 12L21 16.74L16.74 21L12 16.24L7.26 21zm9-3.33l4.74 4.75l1.42-1.42L13.41 12l4.75-4.74l-1.42-1.42L12 10.59L7.26 5.84L5.84 7.26L10.59 12l-4.75 4.74l1.42 1.42z" />
                                        </svg>
                                    </div>
                                </div>
                                {video ? (
                                    ...
                                    <div className="text-center text-white">
                                        {language === "ar-SA" ? "لا يوجد فيديو." : "There is no video."}
                                    </div>
                                )}
                            </>
                        )}
